...
import { collection, addDoc, query, where, getDocs, Firestore, CollectionReference, DocumentData } from "firebase/firestore";
import { userCollection } from "../constants";
import { base_url } from "../env";

class User {
    constructor(public userName: string, public email: string, public userId: string) {}
...
class UserService {
    ...
    async getUserByUserName(userName: string): Promise<User | null> {
        const q = query(this.userCollection, where(userCollection.fields.userName, "==", userName));
    ...
    async getUserByEmail(email: string): Promise<User | null> {
        const q = query(this.userCollection, where(userCollection.fields.email, "==", email));
        const querySnapshot = await getDocs(q);
        if (querySnapshot.size === 0) {
            return null;
        }
